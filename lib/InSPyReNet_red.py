...
class InSPyReNet_red(nn.Module):
    def __init__(self, backbone,in_channels, depth=64, base_size=[384, 384], threshold=512, **kwargs):
      ...
      #  self.model=pretrained_model
        ...
        self.decoder = PAA_d(self.depth * 2, depth=self.depth, base_size=base_size, stage=1)
    ...
    def to(self, device):
        ...
        super(InSPyReNet_red, self).to(device)
        return self
    ...
    def train(self, mode=True):
        super(InSPyReNet_red, self).train(mode)
        self.forward = self.forward_train
        return self
    
    def eval(self):
        super(InSPyReNet_red, self).train(False)
        self.forward = self.forward_inference
        return self
    ...
    def forward_inspyre(self, x):
        ...
        x1, x2, x3, _, _ = self.backbone(x) ## only two outputs for different stages of the backbone
        
        x1 = self.context1(x1) #4
        x2 = self.context2(x2) #4
      #  x3 = self.context3(x3) #8
       # x4 = self.context4(x4) #16
       # x5 = self.context5(x5) #3
        x1 = self.res(x1, (H // 2, W // 2))
        f1, d1 = self.decoder([x1, x2]) #4
                
        # x2 = self.res(x2, (H // 2, W // 2))
        # f2 = self.res(f2, (H // 2,  W // 2 )) ##
        # f1, p1 = self.attention1(torch.cat([x2, f2], dim=1), d2.detach())
        ...
        f1 = self.res(f1, (H, W))
        _, p0 = self.attention0(f1, d1.detach()) #2
...
def InSPyReNet_Res2Net50_red(depth, pretrained, base_size, **kwargs):
...
#    model=eval(opt.Model.name)(**opt.Model)
 #   model_weights="/media/rtiwari/raj/inspyrenet_new/InSPyReNet/data/resnet_ever/latest.pth"
  ...
  #  model.eval()
    return InSPyReNet_red(res2net50_v1b_26w_4s(pretrained=pretrained),[64, 256, 512, 1024, 2048], depth, base_size, **kwargs)
