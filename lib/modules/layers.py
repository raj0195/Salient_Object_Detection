...
def custom_pad(x, b_1=0, b_2=0, h_1=0, h_2=0,w_1=0,w_2=0):
    batch_size, channels, height, width = x.size()
    
    # Create a new tensor with the desired padded dimensions
    padded_x = torch.zeros(batch_size+b_1+b_2, channels, height + h_1 + h_2, width + w_1 + w_2)
    
    # Copy the original content to the center of the padded tensor
    padded_x[:, :, h_1:h_2 + height, w_1:w_2 + width] = x
...
class ImagePyramid:
    ...
    def expand(self, x):
     ...
     #   x=x.cuda()
        #x= custom_pad(x,0,0,self.ksize//2,self.ksize//2,self.ksize//2,self.ksize//2)
        x = F.pad(x, (self.ksize // 2, ) * 4, mode='constant')
